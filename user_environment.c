...
void pd_clear_page_dir_entry(struct Env* env, void* va);
...
//       {
//           uint32 *pt = NULL;
//           get_page_table(e->env_page_directory, (void *)va, &pt);
//
//           if (pt != NULL)
//           {
//               struct Frame_Info *pt_frame = get_frame_info(e->env_page_directory, (void *)va, NULL);
//               if (pt_frame != NULL)
//               {
//                   pd_clear_page_dir_entry(e, (void *)va);
//                   free_frame(pt_frame); // free the page table frame
//               }
//           }
//       }

//remove pages from page file
    int n_pages = ROUNDUP(USER_TOP, PAGE_SIZE) / PAGE_SIZE;

         for (int i = 0; i < n_pages; i++) {
             uint32 va = i * PAGE_SIZE;
...
//5
	...
	free_environment(e);
	//========================/
	cprintf("Done with this function\n");
}
/*void __env_free_with_buffering(struct Env *e)
{
    // 1. Free all buffered pages in user space
    for (uint32 va = 0; va < USER_TOP; va += PAGE_SIZE)
    {
        uint32 *pt = NULL;
        struct Frame_Info *frame = get_frame_info(e->env_page_directory, (void *)va, &pt);

        if (frame == NULL || frame->isBuffered == 0)
            continue;
        ...
        else
        {
            frame->isBuffered = 0;
            frame->environment = NULL;
            LIST_INSERT_HEAD(&free_frame_list, frame);
        }

        unmap_frame(e->env_page_directory, (void *)va); // safely clears PT entry too
    }

    // 2. Free all pages in the working set
    for (int i = 0; i < e->page_WS_max_size; i++)
    {
        if (e->ptr_pageWorkingSet[i].empty == 0)
        {
            void *va = (void *)ROUNDDOWN(e->ptr_pageWorkingSet[i].virtual_address, PAGE_SIZE);
            unmap_frame(e->env_page_directory, va);
        }
    }

    // 3. Free working set memory
#if USE_KHEAP == 1
    kfree(e->ptr_pageWorkingSet);
    e->ptr_pageWorkingSet = NULL;
#endif

    // 4. Free all page tables in the user space
    for (uint32 va = 0; va < USER_TOP; va += PAGE_SIZE * 1024)
    {
        uint32 *pt = NULL;
        get_page_table(e->env_page_directory, (void *)va, &pt);
    ...
    // 5. Remove user pages from the page file
    __remove_pws_user_pages(e);

    // 6. Free the environment
    pf_free_env(e);
    free_environment(e);
